...
import { PanelLeft, Columns, Grid, Minus, Equal, Plus, Sun, Moon, BookOpen, Smile, Frown, Meh, Globe, ThumbsUp, ThumbsDown, Minus as MinusIcon, Focus, Keyboard } from 'lucide-react';
  ...
  lexicalItems.forEach(item => {
    ...
    finalNodes.forEach(node => {
      ...
      matches.forEach((match, matchIndex) => {
        ...
        newNodes.push(
          <LexicalItem key={`${item.id}-${matchIndex}`} item={item} showColors={true}>
            {matchedText}
          </LexicalItem>
        );
        ...
        <div className="flex items-center justify-center space-x-2 md:space-x-4">
          ...
          <div className="flex items-center space-x-1">
            <button 
              ...
              className={`p-2 rounded-md ${focusMode ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}`}
              title="Focus Mode (F)"
            >
              <Focus size={20} />
            </button>
